...
func (c *VideoController) Find(ctx *gin.Context) {
	...
	videos, err := c.service.Find(filter)
	if err != nil {
...
func (c *VideoController) Get(ctx *gin.Context) {
	...
	video, err := c.service.Get(videoId)
	if err != nil {
		httputil.NewError(ctx, http.StatusNotFound, err)
		return
	}
...
func (c *VideoController) Add(ctx *gin.Context) {
	...
	video, err := c.service.Add(addVideo)
	if err != nil {
		httputil.NewError(ctx, http.StatusNotFound, err)
		return
	}
...
func (c *VideoController) Update(ctx *gin.Context) {
	...
	video, err := c.service.Update(videoId, updateVideo)
	if err != nil {
		httputil.NewError(ctx, http.StatusNotFound, err)
		return
	}
...
func (c *VideoController) Delete(ctx *gin.Context) {
	...
	_, err = c.service.Remove(videoId)
	if err != nil {
		httputil.NewError(ctx, http.StatusNotFound, err)
		return
	}
